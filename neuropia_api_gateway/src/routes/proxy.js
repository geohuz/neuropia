...
router.all('/*', async (req, res) => {
    try {
        ...
        const portkeyResponse = await callPortkeyGateway(portkeyConfig, requestBody, userContext, originalPath);
...
async function callPortkeyGateway(config, requestBody, userContext, path) {
    ...
    if (!validation.success) {
        ...
        throw new Error(`Invalid Portkey configuration: ${validation.error.issues[0].message}`);
    }
