...
```
payment_jobs (支付任务) → fund_transactions (资金流水) → account_balance (余额快照)
```
...
BEGIN
    -- ✅ 参数验证
    IF p_amount <= 0 THEN
        RAISE EXCEPTION '充值金额必须大于0，实际值: %', p_amount;
    END IF;
    ...
    DO UPDATE SET 
        owner_userid = EXCLUDED.owner_userid  -- 触发ON CONFLICT
    RETURNING id, balance 
    INTO v_account_id, v_old_balance;
...
参数说明：
- p_user_id: 用户UUID，必填
- p_amount: 充值金额，必须大于0，numeric(15,6)
- p_currency: 货币代码，如"CNY"、"USD"
- p_payment_intent_id: 支付订单号，关联payment_jobs.payment_intent_id
- p_channel: 支付渠道，如"alipay"、"wechat_pay"
- p_provider_tx_id: 第三方支付交易号，用于对账
- p_metadata: 扩展信息，JSON格式，默认空对象

返回值：fund_transactions.id（资金流水记录ID）

功能：
1. 锁定用户账户余额行
2. 插入资金流水记录到fund_transactions
3. 更新账户余额到account_balance

调用示例：
SELECT data.record_payment_deposit(
    ''a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11'', -- 用户ID
    100.00,                                  -- 充值金额
    ''CNY'',                                   -- 货币
    ''pay_intent_202401010001'',               -- 支付订单号
    ''alipay'',                                -- 支付渠道
    ''2024010123456789'',                      -- 支付宝交易号
    ''{"order_id": "ORD123"}''::jsonb          -- 扩展信息
);
...
### **3. 记账函数执行**
```sql
-- record_payment_deposit内部：
-- 1. 锁定account_balance
-- 2. 插入fund_transactions
-- 3. 更新account_balance
-- 返回transaction_id
```
...
fund_transactions:
    新增一条deposit记录，balance_before→balance_after
...
### **核心原则**
- **支付状态** → payment_jobs表管理
- **资金记账** → record_payment_deposit函数处理
- **余额查询** → account_balance表直接读取
...
| consume      | t1   |                            |                  |                                | -10 -> 90     | 10                             |
| consume      | t2   |                            |                  |                                | -30 -> 60     | 30                             |
| topup 20     | t3   | see balance from redis: 60 | 20               | 120                            | 60+20 -> 80   | 写入一条特殊记录以反映最新余额 |
