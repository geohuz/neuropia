...
```postgresql
-- 针对 DISTINCT ON (account_id) ORDER BY created_at DESC 优化
CREATE INDEX idx_usage_log_account_created_desc 
ON data.usage_log (account_id, created_at DESC) 
INCLUDE (balance_after)  -- PostgreSQL 11+ 支持INCLUDE
WHERE sync_status = 'completed';
```

## 同步log表: data.balance_sync_log
...
## 同步函数: data.sync_balance

更新:

1. `data.account_balance`
2. `data.balance_sync_log`
...
DECLARE
    ...
    updated_count INT;
BEGIN
    -- 每个账户取最新的一条balance_after，更新到account_balance
    WITH latest_balances AS (
...
```postgresql
select * from cron.job_run_details;  -- 查看任务运行细节

UPDATE cron.job SET active = false;  -- 停止所有任务

select * from cron.job;              -- 查看任务
```
