...
1. **加 JWT → user_id 注入 userContext**（现在没有 user_id）
2. **实现价格矩阵（不同 provider/model/customer_type）**
3. **实现 balanceSyncWorker → DB 写回**
4. **实现每日/每小时账单聚合 table**（audit log）
5. **加限流**

做完这几个，你的 API Gateway 就是一个真正商业化平台。
...
- 🧱 完整实现 JWT userContext 结构
- 💰 完整账单表 schema + 写回 worker
- 📈 token 消耗 price matrix 设计
- ⚙️ Redis 限流（漏桶/令牌桶）
