...
1. **API Gateway**
   - 实时 **从 Redis 扣费**（chargeUser / chargeAccount）
   - 实时返回给用户结果
   - 保证业务链路中 **余额检查 + 扣费** 都在同一进程里完成
2. **回写数据库**
   - 可以 **单独一个异步服务**去做
   - 从 Redis 或队列表中拉取变动
   - 写入 Postgres `account_balance`
   - 写回触发的 `pg_notify` 只会刷新缓存，不会造成无限循环

这样做有几个好处：

- API Gateway 不受写 DB 延迟影响，响应更快
- 扣费逻辑仍然集中，安全可靠
- 回写服务可以独立扩展，批量处理，提高性能

💡 关键点：

- **Redis 扣费是权威数据**
- **写回 DB 是异步同步**，只要 notify 机制正常，缓存永远一致

所以结论是：**扣费放在 API Gateway，回写分开是合理且推荐的设计**。
